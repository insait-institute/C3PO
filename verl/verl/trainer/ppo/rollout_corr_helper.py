...
def _parse_rollout_is_threshold(threshold_spec: str | float) -> tuple[float, Optional[float]]:
    if isinstance(threshold_spec, bool):
        raise TypeError("rollout_is_threshold must be specified as a float or a string threshold specification, not a boolean.")
...
def _parse_rollout_rs_thresholds(options: list[str], threshold_spec: Optional[str | float]) -> dict[str, dict[str, Optional[float]]]:
    ...
    if len(raw_specs) not in (1, len(options)):
        raise ValueError(f"rollout_rs_threshold expects either one threshold shared by all options or exactly {len(options)} thresholds to match the provided rollout_rs options.")
    ...
    for option, spec in zip(options, raw_specs, strict=False):
        if option.endswith("k1"):
            ...
            if "_" in spec:
                raise ValueError(f"rollout_rs_threshold for option '{option}' must provide a single upper bound without '_'. Received '{spec}'.")
            try:
                upper = float(spec)
            except ValueError as exc:
                raise ValueError(f"Invalid numeric threshold '{spec}' for option '{option}'.") from exc
...
def compute_rollout_rejection_mask(
    ...
    for opt in option_modes:
        if opt not in SUPPORTED_ROLLOUT_RS_OPTIONS:
            raise ValueError(f"Invalid rollout_rs option: {opt}. Must be one of {sorted(SUPPORTED_ROLLOUT_RS_OPTIONS)}.")
        if opt not in seen:
    ...
    for option_name in normalized_options:
        ...
        if is_k1_option:
            if lower_value is None or upper_value is None:
                raise ValueError(f"rollout_rs_threshold for option '{option_name}' must specify both lower and upper bounds.")
        ...
        if option_name == "token_k1":
            ...
            if option_name.endswith("k1"):
                if lower_log is None:
                    raise ValueError(f"Threshold specification for option '{option_name}' must include lower and upper bounds.")
...
def compute_rs_metrics(
    ...
    *,
    ...
    if level == "sequence" and seq_values_valid.numel() > 0:
        ...
        fraction_low = (seq_values_valid < rollout_rs_threshold_lower).float().mean().item() if apply_lower_threshold else 0.0
    else:
        fraction_high = verl_F.masked_mean((rs_statistic > rollout_rs_threshold).float(), response_mask).item()
        fraction_low = verl_F.masked_mean((rs_statistic < rollout_rs_threshold_lower).float(), response_mask).item() if apply_lower_threshold else 0.0
    ...
    if seq_values_valid.numel() > 0:
        ...
        if apply_lower_threshold:
            metrics[f"{prefix}_seq_fraction_low"] = (seq_values_valid < rollout_rs_threshold_lower).float().mean().item()
    else:
...
def compute_rollout_correction_weights(
    ...
    if rollout_is == "token":
        ...
        log_ratio_sum: torch.Tensor = verl_F.masked_sum(log_ratio, response_mask, axis=-1).unsqueeze(-1)  # Shape: (batch_size, 1)
    ...
    if not use_icepop:
        ...
        token_kept_mask = (raw_rollout_is_weights >= rollout_is_threshold_lower) & (raw_rollout_is_weights <= rollout_is_threshold_upper)
        rollout_is_weights = torch.where(token_kept_mask, raw_rollout_is_weights, torch.zeros_like(raw_rollout_is_weights))
    ...
    if use_icepop:
        assert rollout_is_threshold_lower is not None
        oob_mask = (raw_rollout_is_weights < rollout_is_threshold_lower) | (raw_rollout_is_weights > rollout_is_threshold_upper)
...
def compute_is_metrics(
    ...
    rollout_is_threshold_lower = 1.0 / rollout_is_threshold if rollout_is_threshold_lower is None else rollout_is_threshold_lower
    ...
    if rollout_is == "sequence":
        ...
        metrics["rollout_is_ratio_fraction_high"] = verl_F.masked_mean(rollout_is_above_threshold.float(), response_mask).item()
        metrics["rollout_is_ratio_fraction_low"] = verl_F.masked_mean(rollout_is_below_threshold.float(), response_mask).item()
    ...
    if mask_count > 1:
        ...
        rollout_is_var: torch.Tensor = verl_F.masked_mean(weights_for_std.square(), response_mask) - mean_clamped.square()
    ...
    metrics["rollout_is_eff_sample_size"] = 1.0 / verl_F.masked_mean(is_weights_normalized.square(), response_mask).item()
...
def compute_rollout_correction_and_rejection_mask(
    ...
    if old_log_prob.shape != rollout_log_prob.shape:
        raise ValueError(f"old_log_prob shape {old_log_prob.shape} does not match rollout_log_prob shape {rollout_log_prob.shape}.")
    if old_log_prob.shape != response_mask.shape:
        raise ValueError(f"log_prob shape {old_log_prob.shape} does not match response_mask shape {response_mask.shape}.")
    ...
    if rollout_rs is not None:
        if rollout_rs_threshold is None:
            raise ValueError("rollout_rs_threshold must be explicitly provided when rollout_rs is enabled. Set rollout_rs_threshold to the desired threshold value.")
...
def compute_rollout_correction_and_add_to_batch(batch: DataProto, rollout_corr_config: RolloutCorrectionConfig) -> tuple[DataProto, dict]:
...
def apply_bypass_mode(
    ...
    if "rollout_log_probs" not in batch.batch:
        raise ValueError("bypass_mode=True requires rollout_log_probs in batch. Ensure rollout worker is configured to calculate_log_probs=true.")
