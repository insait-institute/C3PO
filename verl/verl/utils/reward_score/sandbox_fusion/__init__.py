...
def compute_score(
    ...
    Args:
        ...
        retry_on_timeout: Whether sandbox read timeouts / connection errors are
            retried with backoff instead of immediately scoring the case as failed.
    ...
    try:
        ...
        res_list, metadata_list = check_correctness(
            ...
            # Binary mode can stop at the first failing case; continuous mode
            # needs every (first-N) case run to compute a pass-rate.
            short_circuit=not continuous,
        )
        ...
        if continuous:
            ...
            # Binary verdict: a solution is correct iff EVERY test case passes.
            # check_correctness short-circuits on the first failure, so res_list
            # may hold None for cancelled cases -- any non-True entry (wrong
            # answer, error, or cancelled) makes the verdict 0.0.
            score = 1.0 if res_list and all(r is True for r in res_list) else 0.0
            final_metadata = metadata_list
