...
import regex
import sympy
from constants import complete_mapper, manual_mapper
from sympy import Mul, N, Symbol, simplify
...
def latex2sympy_fixed(latex: str):
    ...
    expr = latex_parsed.xreplace({s: known_constants[s.name] for s in latex_parsed.free_symbols if s.name in known_constants})
    return expr
...
def parse_answer(s: str, primitive_type: type = None, list_answer: bool = False):
    ...
    if not list_answer and "," not in s:
        parsed, warning = ParsePrimitive.parse(s, primitive_type=primitive_type)
...
def check_answers(ans1, ans2):
    ...
    if (type(ans1) in [list, AnswerList]) != (type(ans2) in [list, AnswerList]):
        return False
    try:
        if not (hasattr(ans1, "equals") and callable(ans1.equals)) or not (hasattr(ans2, "equals") and callable(ans2.equals)):
            if isinstance(ans1, (str, int)) or isinstance(ans2, (str, int)):
                # sympy check equality
                return bool(ans1 == ans2)
            n1, n2 = N(ans1), N(ans2)
            if not n1.is_finite or not n2.is_finite:
                return bool(n1 == n2)
    ...
    def parse(cls, string, primitive_type):
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Error: Custom parsing error {e}, {string_no_eq}")
...
class ParseList(ParseObject):
    """A class for parsing lists with protection against infinite nesting."""

    @classmethod
    def is_at_start(cls, string):
        # Must start with '(' AND contain a comma at the top level to be a 'List'
        if not string.startswith("("):
            return False

        # Count nesting to find if a comma exists OUTSIDE of inner parentheses
        depth = 0
        for char in string:
            if char == "(":
                depth += 1
    ...
    @classmethod
    def is_finished(cls, string):
        # A list is finished if it ends with a closing parenthesis
        return string.strip().strip(",").endswith(")")

    @classmethod
    def is_complete(cls, string):
        # Standard parenthesis balancing
        return string.count("(") == string.count(")")

    @classmethod
    def never_zero_count(cls, string):
        """Checks if the parenthesis count never reaches zero before the end."""
        count = 0
        for char in string:
            if char == "(":
                count += 1
    ...
    def parse(cls, string, delimiter=[r"\n", ","], primitive_type=None, depth=0):
        ...
        if depth > 20:
            logger.error(f"Max recursion depth reached at: {string[:50]}")
            return None, WarningType.MAJOR

        output = []
        warning = WarningType.NONE

        # 1. Strip outer matching brackets: ((1,2)) -> (1,2)
        string = string.strip().strip(",")
        if len(string) > 2 and cls.never_zero_count(string[:-1]):
            string = string[1:-1]
        ...
        while len(string) > 0:
            ...
            # Prioritize List parsing only if there's a delimiter or nested list
            # Otherwise, ParsePrimitive handles expressions like (a+5)(b+2)
            allowed_objects = [ParseList, ParsePrimitive]

            found_match = False
            for obj in allowed_objects:
                if not obj.is_at_start(strip(string)):
                    continue

                current_index = 1
                while current_index <= len(comma_separated):
                    candidate = strip(used_delim.join(comma_separated[:current_index]))

                    if obj.is_complete(candidate) and obj.is_finished(candidate):
                        # Recursive call
                        if obj == ParseList:
                            parsed, new_warning = obj.parse(candidate, delimiter=delimiter, primitive_type=primitive_type, depth=depth + 1)
                        else:
                            parsed, new_warning = obj.parse(candidate, primitive_type=primitive_type)

                        # CRITICAL FIX: Only consume string and append if parsing succeeded
                        if parsed is not None:
                            output.append(parsed)
                            warning = max(warning, new_warning)
                            # Remove the parsed part from the remaining string
            ...
            # 4. Stall protection: If no object was extracted, break to avoid infinite loop
            if not found_match or string == previous_string:
                # If we have some items (e.g., "1, 2, stuck_here"), return the partial list
                # If we have nothing, return None so the parent doesn't wrap an empty list
                if not output:
                    return None, WarningType.MAJOR
                break
